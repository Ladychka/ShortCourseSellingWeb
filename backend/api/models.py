...
try:
    from moviepy.editor import VideoFileClip  # heavy optional dependency
...
class Course(models.Model):
    ...
    image=models.FileField(upload_to="course-image", blank=True, null=True)
    title=models.CharField(max_length=200)
    description=models.TextField(null=True, blank=True)
    price=models.DecimalField(max_digits=12, decimal_places=2, null=True, blank=True)
    language = models.CharField(max_length=50, default="English")
    level = models.CharField(max_length=50, default="Beginner")
    platform_status = models.CharField(max_length=50, default="active")
    teacher_status = models.CharField(max_length=50, default="available")
    featured = models.BooleanField(default=False)
    course_id = ShortUUIDField(unique=True, max_length=20, length=6, alphabet="0123456789")
    slug = models.SlugField(unique=True, blank=True, null=True)
...
class VariantItem(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        if self.file and MOVIEPY_AVAILABLE:
            try:
                clip = VideoFileClip(self.file.path)
                duration_seconds = clip.duration or 0
                ...
                # Silently ignore video metadata extraction failures
                pass
...
class CartOrderItem(models.Model):
    ...
    course=models.ForeignKey(Course, on_delete=models.CASCADE, related_name="orderitem")
    teacher=models.ForeignKey(Teacher, on_delete=models.CASCADE)
    tax_fee=models.DecimalField(max_digits=12,default=0.00, decimal_places=2)
    price=models.DecimalField(max_digits=12,default=0.00, decimal_places=2)
    total=models.DecimalField(max_digits=12,default=0.00, decimal_places=2)
...
class Wishlist(models.Model):
    ...
    def __str__(self):
        return f"{self.user} - {self.course}" if self.user else self.course.title
